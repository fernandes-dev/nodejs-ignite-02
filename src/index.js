...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const freePlanNotAuthorized = !user.pro && user.todos.length >= 10;

  if (freePlanNotAuthorized)
    return response
      .status(403)
      .json({ error: "Upgrade your plan to continue" });
...
function checksTodoExists(request, response, next) {
  ...
  if (!findedUser.todos || findedUser.todos.length === 0)
    return response.status(404).json({ error: 'Not authorized' });

  const todo = findedUser.todos.find((todo) => todo.id === id);
